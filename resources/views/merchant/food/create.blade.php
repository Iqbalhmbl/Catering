...
<div class="container">
    <div class="row">
        ...
        <div class="col-md-9 mb-4">
            <h1 class="mb-4">Tambahkan Menu Makanan</h1>
            <form action="{{ route('foods.store') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    ...
                    <input type="text" class="form-control" id="food_name" name="food_name" value="{{ old('food_name') }}" required>
                    @error('food_name')
                        <div class="text-danger">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="form-group">
                    ...
                    <select class="form-control" id="type_of_food" name="type_of_food" required>
                        ...
                        <option value="Sayuran" {{ old('type_of_food') == 'Sayuran' ? 'selected' : '' }}>Sayuran</option>
                        <option value="Buah-buahan" {{ old('type_of_food') == 'Buah-buahan' ? 'selected' : '' }}>Buah-buahan</option>
                        <option value="Makanan Berat" {{ old('type_of_food') == 'Makanan Berat' ? 'selected' : '' }}>Makanan Berat</option>
                        <option value="snack" {{ old('type_of_food') == 'snack' ? 'selected' : '' }}>Snack</option>
                        <option value="minuman" {{ old('type_of_food') == 'minuman' ? 'selected' : '' }}>Minuman</option>
                    </select>
                    @error('type_of_food')
                        <div class="text-danger">{{ $message }}</div>
                    @enderror
                ...
                <div class="form-group">
                    ...
                    <input type="text" class="form-control" id="portions" name="portions" value="{{ old('portions') }}" required>
                    @error('portions')
                        <div class="text-danger">{{ $message }}</div>
                    @enderror
                </div>
